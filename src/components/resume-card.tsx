...
export const ResumeCard = ({
  ...
  return (
    <Link
      ...
      onClick={handleClick}
    >
      <Card className="relative overflow-hidden bg-white/5 dark:bg-white/3 backdrop-blur-md border border-white/30 dark:border-white/20 hover:border-white/50 dark:hover:border-white/40 transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-white/5 group-hover:scale-[1.02]">
          ...
          <div className="flex-grow ml-6 items-center flex-col">
            <CardHeader className="p-0">
              <div className="flex items-center justify-between gap-x-2 text-base">
                <h3 className="inline-flex items-center justify-center font-bold leading-none text-sm sm:text-base">
                  {title} 
                  {badges && (
                    <span className="inline-flex gap-x-1 ml-2">
                      {badges.map((badge, index) => (
                        <Badge
                          variant="secondary"
                          className="align-middle text-xs bg-white/15 dark:bg-white/10 border border-white/30 hover:bg-white/25 dark:hover:bg-white/20 transition-colors backdrop-blur-md"
                          key={index}
                        >
                          {badge}
                        </Badge>
                  ...
                  <ChevronRightIcon
                    className={cn(
                      "size-4 translate-x-0 transform opacity-70 transition-all duration-300 ease-out group-hover:translate-x-1 group-hover:opacity-100 ml-2",
                      isExpanded ? "rotate-90" : "rotate-0"
                    )}
                  />
                </h3>
                <div className="text-xs sm:text-sm tabular-nums text-muted-foreground/80 text-right font-medium">
                  {period}
                </div>
               
              </div>
              {subtitle && <div className="font-sans text-sm text-muted-foreground/90 mt-2">{subtitle}</div>}
            </CardHeader>
                    ...
                    <div className="grid gap-3">
                      {projects.map((project, index) => (
                        <div key={index} className="group/project p-4 rounded-xl bg-white/10 dark:bg-white/5 border border-white/25 dark:border-white/15 hover:border-white/35 dark:hover:border-white/25 transition-all duration-300 hover:shadow-lg backdrop-blur-md">
                          <h5 className="font-semibold text-sm mb-2 text-foreground/90 group-hover/project:text-foreground transition-colors">{project.name}</h5>
                          <p className="text-xs text-muted-foreground/80 leading-relaxed">{project.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
